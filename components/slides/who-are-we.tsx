		...
		<article className="flex w-full h-full flex-col lg:flex-row gap-y-4 justify-center items-center">
			<div className="w-full flex flex-col justify-center items-center text-center gap-y-10 px-[5%]">
				<h2 className="text-2xl lg:text-4xl">
					...
					<span className="font-bold text-blue-600">AI agents</span> for
					businesses for over a{" "}
					<span className="font-bold text-blue-600">decade !</span>
				</h2>
				<p className="text-normal lg:text-2xl">
					We decided in 2022 to build an agent who will do our job for us.
				</p>
				<p className="text-normal lg:text-2xl">
					An <span className="text-[#2563eb]">agent</span> that builds agents !
				</p>
			</div>
			<div className="w-full flex flex-col justify-center items-center">
				<div className="w-full grid grid-cols-3 gap-4 items-stretch justify-stretch">
					{prevClients.map((client) => (
						<Card
							key={client.name}
							className="p-2 lg:p-4 flex flex-col justify-center items-center gap-2 shadow-md"
						>
							<Image
								src={client.logo}
								alt={client.name}
				...
				<h3 className="mt-4 text-xl lg:text-2xl">And many others ...</h3>
			</div>
		</article>
	);
}
